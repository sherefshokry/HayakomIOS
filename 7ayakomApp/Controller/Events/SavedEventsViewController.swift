...
class SavedEventsViewController: UIViewController {
    ...
    func loadData(){
           ...
           showIndicator()
           dataSource.getFavouriteMeetUps { (status, response) in
               self.hideIndicator()
               switch status{
               case .sucess :
...
extension SavedEventsViewController : IndicatorInfoProvider{
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "المحفوظة".localize())
    }
    
    
}
extension SavedEventsViewController : UITableViewDataSource,UITableViewDelegate {
